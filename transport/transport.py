...
class Transport:
    ...
    def training_losses(
        ...
        - input_x1: input latent datapoint
        - target_x1: target output latent datapoint
        - model_kwargs: additional arguments for the model
        """
        if model_kwargs == None:
            model_kwargs = {}
        t, target_x0, target_x1 = self.sample(target_x1)
        t, target_xt, target_ut = self.path_sampler.plan(t, target_x0, target_x1)
        model_output = model(input_x1, input_clip_x1, target_xt, t, **model_kwargs)
        B = len(target_x0)

        terms = {}
        # terms['pred'] = model_output
        if self.model_type == ModelType.VELOCITY:
            if isinstance(target_x1, (list, tuple)):
                assert len(model_output) == len(target_ut) == len(target_x1)
                for i in range(B):
                    assert model_output[i].shape == target_ut[i].shape == target_x1[i].shape, (
                        f"{model_output[i].shape} {target_ut[i].shape} {target_x1[i].shape}"
                    )
                terms["task_loss"] = th.stack(
                    [((target_ut[i] - model_output[i]) ** 2).mean() for i in range(B)],
                    dim=0,
                )
            else:
                terms['task_loss'] = mean_flat(((model_output - target_ut) ** 2))
        else:
